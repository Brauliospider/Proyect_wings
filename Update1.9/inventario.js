...
function renderInventory() {
    ...
    filteredProducts.forEach(product => {
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-900">$${product.cost.toFixed(2)}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
...
function updateInventoryMetrics() {
    ...
    const totalValue = inventory.reduce((sum, product) => sum + (product.stock * product.cost), 0);
...
function handleProductForm(event) {
    ...
    const cost = parseFloat(elements.productCost.value);
...
function exportInventory() {
    ...
    csvContent += "Nombre,Categoría,Stock,Stock Mínimo,Costo,Precio,Caducidad\n";
...
document.getElementById("product-form").addEventListener("submit", function (e) {
    ...
    const product = {
        ...
        expiry: document.getElementById("product-expiry").value.trim() || null
    };

    // Llamamos a la función para guardar en la base de datos
    saveProductToDB(product);

    // Cierra el modal después de guardar (opcional)
