  ...
  fetch('conexion.php', {
    ...
    .then(data => {
      if (data.success) {
        ...
        if (data.actualizaciones_inventario && data.actualizaciones_inventario.length > 0) {
          mostrarNotificacionStock(data.actualizaciones_inventario);
        }
        ...
        printTicketDirect(cart, total);
        cart = [];
        updateCart();
        
        showNotification("Venta registrada y stock actualizado correctamente");
...
function printTicketDirect(cartItems, total) {
  const content = `
    <h2 style="text-align:center;">Ticket de Compra</h2>
    <div>
      ${cartItems.map(item => `
  ...
  printWindow.document.write('<style>body{font-family:sans-serif; padding:20px;} h2{text-align:center;}</style>');
...
// Estilo para cantidad con scroll
function aplicarEstiloCantidadScroll() {
  const estilo = document.createElement('style');
  estilo.textContent = `
    .cantidad-scroll::-webkit-scrollbar {
      height: 4px;
    }
    .cantidad-scroll::-webkit-scrollbar-track {
      background: #f3f4f6;
    }
    .cantidad-scroll::-webkit-scrollbar-thumb {
